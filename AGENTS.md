...
- `prom-parse.ts` — pure Prometheus text-exposition parser (`parsePromText` → `PromSample[]`, `selectSample` by name + label superset). Only used by the `prometheus` check type. `fetchHealth` returns `FetchResult.metrics` (UP = reachable + 2xx); `runCheck` loads that monitor's `metric_rules`, reads each rule's value, stores one `metric_samples` row per rule (per-check time series, charted on the detail page), and feeds `MetricInput[]` to the rule engine. A breach opens a `metric` incident (crit-before-warn) but does NOT mark the monitor down. Metric thresholds live in the `metric_rules` table (per rule: metricName + labelMatchers + operator gt/gte/lt/lte + warn/crit) — NOT in `alert_settings`, so they're outside the `EffectiveThresholds` checklist. CRUD at `/api/monitors/[id]/metric-rules`.
