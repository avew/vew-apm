...
- `auth-header.ts` — `buildAuthHeaders(a)` → the request auth header(s) from `a.authType` (none/basic/header/bearer) + `authUsername`/`authHeaderName`/`authHeaderValue`. Takes any `MonitorAuth`-shaped row, so it serves both `monitors` and `metric_sources`. Used by `fetchHealth`, `/api/monitors/sample`, and metric-source scraping. The secret (`authHeaderValue`) is stored **plaintext** (pre-existing; encrypting it is a known follow-up) and never shipped to the client for edit (blank = keep, and it's its own column so an omitted PATCH field keeps it) — same convention on `metric_sources`.
- `check-eval.ts` + `jsonpath.ts` — pure evaluators for the generic monitor types. `monitors.type`: `actuator` (default, parse health tree) | `http` (`evaluateHttp`: `statusMatches` + optional `keyword`) | `json` (`evaluateJson`: 2xx + `getByPath(statusPath)` `valueIsUp`) | `prometheus` (scrape a metrics endpoint). `fetchHealth` branches on `type`; http/json/prometheus set `parsed=null` so component/disk/eureka rules simply don't fire, while availability/latency/cert still do. `POST /api/monitors/sample` previews an endpoint for the form. `jsonpath.ts` is dot/bracket only (no `$..`/filters).
- `prom-parse.ts` — pure Prometheus text-exposition parser (`parsePromText` → `PromSample[]`, `selectSample` by name + label superset). Prometheus metrics attach to **any** monitor (not just the `prometheus` type) via **`metric_sources`** (per-monitor `{label,url}` + optional own auth `authType`/`authUsername`/`authHeaderName`/`authHeaderValue`; a Spring microservice monitor can have several — gateway, billing-svc, … — each behind its own token; `authType` none/null inherits the monitor's auth) + **`metric_rules`** (each rule targets a source). Independent of the health check: `runCheck` scrapes each source referenced by an enabled rule once per tick (`scrapeMetrics`, keyed by source id — not URL, since each source can carry its own auth), `computeMetricReadings` (pure) matches each rule to its source's samples, stores one `metric_samples` row per rule (per-check series, charted on the detail page), and feeds `MetricInput[]` to the rule engine. A breach opens a `metric` incident (crit-before-warn) but does NOT mark the monitor down (health drives UP/DOWN). Metric thresholds live in `metric_rules` (metricName + labelMatchers + operator gt/gte/lt/lte + warn/crit) — NOT in `alert_settings`. Rules also carry a trend `mode` (`instant` default = current value; `sustained`/`delta`/`rate` over `windowSeconds`); trend math is a pure module `metric-trend.ts` (`computeTrend`, unit-tested) — the checker loads recent `metric_samples` (joined to `checks.checkedAt`) for trend rules and reduces history+current to one scalar before the (unchanged) `breaches()` compare, so `metric_samples` still stores the raw value. A trend rule stays silent until its window has enough history (`insufficient`). CRUD: `/api/monitors/[id]/metric-sources` + `/metric-rules`; clone one rule to another endpoint via `/metric-rules/[ruleId]/clone {targetSourceId}`, or a source's whole ruleset via `/metric-sources/[sourceId]/clone-rules {targetSourceId}` (same monitor, sibling endpoints share a Spring template). The detail-page form groups rules under their endpoint. A `prometheus`-type monitor auto-seeds one source (`url`=its own URL) on create; the `metric_samples` table is pruned with checks via FK cascade.
