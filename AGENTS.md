...
- `scheduler.ts` — in-process tick loop; liveness state (`started`/`lastTickAt`) kept on `globalThis` behind `Symbol.for` because Next bundles instrumentation and route handlers separately (a module-level `let` would be duplicated per bundle and invisible to `/api/health`). `GET /api/health` (public in `middleware.ts`) reads it: DB `SELECT 1` + tick freshness → 200/503; drives the Docker `HEALTHCHECK`.
